#!/bin/zsh
# chain-step.sh <assets-dir> <clip-name> <start-image> <prompt> [prev-last-png] [resolution] [aspect]
#
...
#   aspect defaults to 9:16 — films are vertical-native; pass 16:9 only for a deliberate
#   desktop-only build.
#
# Server-side failures (~15% of jobs) are unbilled; this script retries the create+wait
# cycle up to 3 times on its own before giving up.
set -e -o pipefail
DIR=$1; NAME=$2; START=$3; PROMPT=$4; PREV=$5; RES=${6:-1080p}; ASPECT=${7:-9:16}
if [[ -z "$DIR" || -z "$NAME" || -z "$START" || -z "$PROMPT" ]]; then
  echo "usage: chain-step.sh <assets-dir> <clip-name> <start-image> <prompt> [prev-last-png] [resolution] [aspect]"; exit 1
fi
...
for ATTEMPT in 1 2 3; do
  echo "[$NAME] creating job ($RES/$MODE/$ASPECT, audio off) — attempt $ATTEMPT..."
  CREATE=$(higgsfield generate create seedance_2_0 \
    --prompt "$PROMPT" \
    --start-image "$START" \
    --duration 5 --resolution "$RES" --mode "$MODE" --aspect_ratio "$ASPECT" \
    --generate_audio false --json 2>&1 || true)
  ...
  if [[ -z "$ID" ]]; then
    echo "[$NAME] could not parse a job id from create output (not guessing from job list). Raw tail:"
    echo "$CREATE" | tail -5
    [[ $ATTEMPT -lt 3 ]] && { sleep 5; continue; } || exit 1
  fi
  ...
  echo "[$NAME] no mp4 url (server-side failures are unbilled) — retrying..."
  echo "$WAIT" | tail -3
done
if [[ -z "$URL" ]]; then echo "[$NAME] FAILED after 3 attempts."; exit 1; fi
